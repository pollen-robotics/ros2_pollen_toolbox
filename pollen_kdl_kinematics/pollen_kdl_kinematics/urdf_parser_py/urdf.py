...
xmlr.reflect(
    JointDynamics, tag="dynamics", params=[xmlr.Attribute("damping", float, False), xmlr.Attribute("friction", float, False)]
)
...
xmlr.reflect(Mesh, tag="mesh", params=[xmlr.Attribute("filename", str), xmlr.Attribute("scale", "vector3", required=False)])


class GeometricType(xmlr.ValueType):
    def __init__(self):
        self.factory = xmlr.FactoryType("geometric", {"box": Box, "cylinder": Cylinder, "sphere": Sphere, "mesh": Mesh})
...
xmlr.reflect(
    JointMimic,
    tag="mimic",
    params=[xmlr.Attribute("joint", str), xmlr.Attribute("multiplier", float, False), xmlr.Attribute("offset", float, False)],
)
...
xmlr.reflect(Actuator, tag="actuator", params=[name_attribute, xmlr.Element("mechanicalReduction", float, required=False)])
