...
class MinimalActionClientAsyncIO(Node):
    ...
    def feedback_callback(self, feedback):
        self.get_logger().warn(
            f"Received feedback: status {feedback.feedback.status} time to completion {feedback.feedback.time_to_completion}"
        )

    async def send_goal_async(self, joints: List[str], goal_positions: List[float], duration: float):
    ...
    def send_goal(self, joints: List[str], goal_positions: List[float], duration: float):
        self.get_logger().info("Waiting for action server...")
        self._action_client.wait_for_server()
        ...
        goal_handle = self._action_client.send_goal(goal_msg, feedback_callback=self.feedback_callback)
...
async def run(args, loop):
    ...
    # my_task1 = loop.create_task(action_client.send_goal_async(['r_shoulder_pitch'], [-1.0], 2.0))
    # my_task2 = loop.create_task(action_client.send_goal_async(['r_shoulder_pitch'], [0.0], 2.0))
    ...
    # # Sequence
    result, status = await loop.create_task(action_client.send_goal_async(["r_shoulder_pitch"], [-1.0], 2.0))
    logger.info(f"A) result {result} and status flag {status}")
    result, status = await loop.create_task(action_client.send_goal_async(["r_shoulder_pitch"], [0.0], 2.0))
    logger.info(f"B) result {result} and status flag {status}")

    # r, s = action_client.send_goal(['r_shoulder_pitch'], [-1.0], 2.0)
    # logger.info(f'result {r} and status flag {s}')
