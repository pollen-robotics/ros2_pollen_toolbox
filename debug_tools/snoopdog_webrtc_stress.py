...
class TeleopApp:
    ...
    def make_arm_cartesian_goal(self, x: float, y: float, z: float, partid=1) -> None:
        goal = np.array(
            [
                [0, 0, 1, x],
                [0, 1, 0, y],
    ...
    def make_neck_goal(self, x: float, y: float, z: float, partid) -> None:
        goal = np.array(
            [
                [0, 0, 1, x],
                [0, 1, 0, y],
    ...
    def ensure_send_command(self, channel: RTCDataChannel, freq: float = 100) -> None:
        async def send_command() -> None:
            print("coucou")
            ## NOTE: these values are feasible by the robot
            ## DO NOT TOUCH
            ...
            while True:
                ...
                target_gripper = 0.5 - 0.5 * np.sin(2 * np.pi * time.time())
                # neck_orientation =

                commands = AnyCommands(
                    commands=[
                        AnyCommand( # right arm
                            arm_command=ArmCommand(arm_cartesian_goal=self.make_arm_cartesian_goal(fixed_x, y-0.2, z,
                                                                                                   partid=self.connection.reachy.r_arm.part_id)),
                        ),
                        AnyCommand( # left arm
                            arm_command=ArmCommand(arm_cartesian_goal=self.make_arm_cartesian_goal(fixed_x, y+0.2, z,
                                                                                                   partid=self.connection.reachy.l_arm.part_id)),
                        ),
                        # AnyCommand( # right gripper
                        #     hand_command=HandCommand(
                        #         hand_goal=HandPositionRequest(
                        #             id=self.connection.reachy.r_hand.part_id,
                        #             position=HandPosition(parallel_gripper=ParallelGripperPosition(position=target_gripper)),
                        #         ),
                        #     ),
                        # ),
                        # AnyCommand( # left gripper
                        #     hand_command=HandCommand(
                        #         hand_goal=HandPositionRequest(
                        #             id=self.connection.reachy.l_hand.part_id,
                        #             position=HandPosition(parallel_gripper=ParallelGripperPosition(position=target_gripper)),
                        #         ),
                        #     ),
                        # ),
